...
st.write("由于对Cinnamon桌面环境的外观不太满意，就决定尝试Budgie桌面环境。在Linux Mint有软件包但存在依赖项冲突，在EndeavourOS安装切换不了桌面环境，Ubuntu有对应第三方发行版但用了一些Ubuntu软件导致外观不一致且桌面环境为外观接近Gnome3的版本，就尝试了Slous Linux。")
st.write("当前版本的Budgie桌面环境用了不少阴影效果（任务栏阴影需要开启内置主题），再加上图标设计，算是Linux上最好的Material Design方案，个人认为在Linux中也是外观最好的设计。另一个优势是比较轻量化，流畅度明显优于Cinnamon，算是在j1800唯一达到可用水平的又动画的桌面环境，n5095以上配置应该能在高分辨率达到比较高的流畅度，1080p分辨率用j1800就比较流畅了。不过功能方面，由于文本编辑器等程序折叠了菜单，功能性不如Cinnamon桌面环境。另外也有不支持非整数缩放和只有左右分屏的功能缺陷。")
st.image("blog/files/bg1.jpg")
st.image("blog/files/bg2.jpg")
st.image("blog/files/bg3.jpg")
st.write("Slous Linux从Debian二次开发但不支持deb软件包，虽然不少知名开源软件可以在软件源找到但有一部分需要另外更换。目前通过clash-for-linux-install项目使用代理，重启后需要手动设置系统代理。Ollama和Chatbox等应用使用安装脚本或appimage就不受影响。Timeshift,Edge和Github Desktop等应用没有对应版本就无法安装。")
