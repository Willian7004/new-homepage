...
st.write("推理模型可以通过增加推理步骤来提高性能。目前最优秀的闭源推理模型是Grok3 Reasoning系列，基本能对标未发布的o3。最优秀的开源推理模型是Deepseek R1，其蒸馏版本在本地部署上有优势，表中用优于官方蒸馏版的蒸馏模型替换了官方版本。由于不少模型不是同时测试，表中只保留了有较多模型进行的测试。beat of 64测试的结果写到括号内。")
