...
st.write("思维链模型使用思维链和多次采样等方法提高性能，大部分方法会只能带来一次提升并且增加推理成本。目前高质量公开数据基本耗尽，但其中的推理数据偏少。使用思维链模型合成数据后，可以训练推理能力更强的基础模型，持续提高模型性能。")
...
st.write("一些有批量生成需求的用户认为Flux.1 dev生成较慢，不过如果是轻度使用，在主流配置时的生成速度足够。Hunyuan Video在主流配置的生成速度比较慢，但使用第三方api平台，例如在Siliconflow的价格是每个视频0.7元（完整长度），大部分用户应该可以接受，商业模型中成本较低的Vidu2.0每个视频约0.3元，推测Cosmos 1.0 Diffusion 14b也接近这一成本。视频模型中成本问题明显的主要是一些参数量较大或架构优化不足的商业模型。")
