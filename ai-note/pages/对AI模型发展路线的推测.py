...
st.write("在主流配置下，Flux.1 dev用于批量生成时较慢，轻度使用速度没问题。Cosmos 1.0 Diffusion 7b图生视频在主流配置速度还可以，更大的模型生成长度和速度都不理想。Hunyuan Video在Siliconflow的定价是每个视频2.8元，非商用价格还是偏高了，推测Step Video和Wan2.1成本也接近。较新的商业模型有一些也考虑了成本问题，效果好且成本相对较低的可灵1.6定价按订阅价格换算约为每秒0.4元（成本更低的Vidu2.0实测效果不佳）。")
